...
function Component_type(props) {
	...
	const { ctkhuyenMaiCreator, khuyenMaiCreator, modalFormCreator, ListLoaiGiay, ListGiay, khuyenMaiEditting } = props;
	...
	function handleformfromdate(date) {
		...
		if (date1 > date2) {
			setShow(true);
			setNd('Vui lòng chọn mốc thời gian tại thời điểm hiện tại hoặc tương lai');
		} else {
	...
	function onchangeSelectLG(e) {
		let da = ListGiay.filter((item) => item.id_loai_giay === parseInt(e.target.value));
		let d = [];
	...
	function deleteSelectGiay(index) {
		...
		setShow(true);
		setNd(`Bạn muốn xóa ${khuyenmai[index].ten_giay} ra khỏi chương trình khuyến mãi`);
	}
	function submit() {
						...
						if (addCTKM.length > 0) {
							for (var i = 0; i < addCTKM.length; i++) {
								const datanews = {
									...
									gia_ban_khuyen_mai:
										addCTKM[i].gia_ban - (addCTKM[i].gia_ban * data.phan_tram) / 100,
								};
								apiCTKM
						...
						if (khuyenmai.length > 0) {
							for (var i = 0; i < khuyenmai.length; i++) {
								const datanews = {
									...
									gia_ban_khuyen_mai:
										khuyenmai[i].gia_ban - (khuyenmai[i].gia_ban * data.phan_tram) / 100,
								};
								apiCTKM
									.ThemCTKhuyenMai(datanews)
									.then((responseCTKM) => {
	...
	useEffect(() => {
		...
		if (khuyenMaiEditting.id) {
			...
			for (var i = 0; i < ListGiay.length; i++) {
				let dataKM = CTKhuyenMaiEditting.filter((item) => item.id_giay === ListGiay[i].id);
				if (dataKM.length > 0) {
	...
	const { ngay_bat_dau, ngay_ket_thuc, ten_khuyen_mai, mo_ta, phan_tram, hinh_anh } = data;
			...
			<form className="row tm-edit-product-form" onSubmit={handleSubmit}>
				<div className="col-xl-6 col-lg-6 col-md-12">
					...
					<div className="form-group mb-3">
						...
						/>
						{errors.ten_khuyen_mai && <p className="error">{errors.ten_khuyen_mai}</p>}
					</div>
					<div className="form-group mb-3">
				...
				<div className="col-xl-6 col-lg-6 col-md-12 mx-auto mb-4">
					...
					<div className="custom-file mt-3 mb-3">
						<input type="file" name="file" accept="image/*" multiple onChange={(e) => handleChange(e)} />
						<button
							type="button"
				...
				<div className="col-12">
					<div className="ct">
						<div className="title-ct-khuyenmai">Lựa chọn giày cho chương trình khuyến mãi</div>
						<div className="content-select-giay">
							{khuyenMaiEditting.id ? (
								boolThemSP === true ? (
									...
									<Button variant="primary" className="padding-km" onClick={boolThemSPSP}>
							...
							<div className="list-giay">
								{g.length > 0 ? (
									<div className="khung-select-giay">
										<div className="title-ct-khuyenmai">Nhấn vào sản phẩm để lựa chọn sản phẩm</div>
									...
									{khuyenmai.map((item, index) => {
										return (
											<div
												...
												className="tiltle-giay"
												title={item.ten_giay}
												onClick={() => deleteSelectGiay(index)}
											>
												{item.ten_giay}
											</div>
										);
