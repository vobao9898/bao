...
function Component_type(props) {
	...
	const { onUpload, onChangeInput, handleChange, handleSubmit, data, setData, errors } = useForm(
		submit,
		validate,
		apiUpload
	);
	...
	function submit() {
		let ktCS = ListLoaiGiay.filter((item) => item.ten_loai_giay !== loaiGiayEditting.ten_loai_giay);
		let k = ktCS.filter((item) => item.ten_loai_giay === data.ten_loai_giay);
		const kt = ListLoaiGiay.filter((item) => item.ten_loai_giay === data.ten_loai_giay);
			...
			<form className="row tm-edit-product-form" onSubmit={handleSubmit}>
				<div className="col-xl-6 col-lg-6 col-md-12">
					<div className="form-group mb-3">
						...
						/>
						{errors.ten_loai_giay && <p className="error">{errors.ten_loai_giay}</p>}
					</div>
					<div className="form-group mb-3">
						<label>Mô tả</label>
						<textarea
