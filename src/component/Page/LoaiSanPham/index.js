...
function LoaiSanPham(props) {
	...
	const { setLoaiGiayEditting, deleteLoaiGiay, updateLoaiGiay, SetTH, filterThuongHieu } = LoaiGiayAtionCrearotors;
				...
				.then((response) => {
					if (response.status === 200) {
						if (response.data.success === 500) {
							setShow(false);
							setNd(`Bạn Không thể xóa loại giày này hãy cập nhật lại trạng thái của loại giày này`);
							setShow(true);
							setDataTam({});
						} else {
	...
	return (
		<div className="product-admin">
			...
			<div className="type_product">
				...
				<Table striped bordered hover variant="dark" className="table_type khuyenmai">
					<thead>
						<tr>
							<th>STT</th>
							<th>Tên loại giày</th>
									...
									return (
										<tr key={index + 1}>
											...
											<td className="Controls_type">
												<Button variant="primary update_type" onClick={() => update(post)}>
													sửa
												</Button>
												<Button
