...
function Component_mau_sac(props) {
	const { onChangeInput, handleSubmit, data, setData, errors, handleChange, onUpload } = useform(
		submit,
		validate,
		apiUpload
	);
	...
	function submit() {
		...
		let ktCS = ListMauSac.filter((item) => item.ten_mau_sac !== mausacEditting.ten_mau_sac);
		let k = ktCS.filter((item) => item.ten_mau_sac === data.ten_mau_sac);
		const kt = ListMauSac.filter((item) => item.ten_mau_sac === data.ten_mau_sac);
			...
			<form className="row tm-edit-product-form" onSubmit={handleSubmit}>
				<div className="col-xl-6 col-lg-6 col-md-12">
					<div className="form-group mb-3">
						...
						/>
						{errors.ten_mau_sac && <p className="error">{errors.ten_mau_sac}</p>}
					</div>
				</div>
